...
export type Query = {
  ...
  conver: ConversionResponse;
  statistics: StatisicsResponse;
};


export type QueryConverArgs = {
...
export type ConverQueryVariables = Exact<{
  amount: Scalars['Float'];
  destination: Scalars['String'];
  source: Scalars['String'];
}>;
...
export type ConverQuery = { __typename?: 'Query', conver: { __typename?: 'ConversionResponse', value: number, source: string, destination: string } };
...
export const ConverDocument = gql`
    query Conver($amount: Float!, $destination: String!, $source: String!) {
  conver(amount: $amount, destination: $destination, source: $source) {
    value
    source
    destination
  }
 ...
 * __useConverQuery__
 *
 * To run a query within a React component, call `useConverQuery` and pass it any options that fit your needs.
 * When your component renders, `useConverQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useConverQuery({
 *   variables: {
 ...
 * });
 */
export function useConverQuery(baseOptions: Apollo.QueryHookOptions<ConverQuery, ConverQueryVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<ConverQuery, ConverQueryVariables>(ConverDocument, options);
      }
export function useConverLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<ConverQuery, ConverQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<ConverQuery, ConverQueryVariables>(ConverDocument, options);
        }
export type ConverQueryHookResult = ReturnType<typeof useConverQuery>;
export type ConverLazyQueryHookResult = ReturnType<typeof useConverLazyQuery>;
export type ConverQueryResult = Apollo.QueryResult<ConverQuery, ConverQueryVariables>;
